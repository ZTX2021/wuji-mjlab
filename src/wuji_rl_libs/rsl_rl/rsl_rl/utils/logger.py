...
class Logger:
    ...
    def init_logging_writer(self) -> None:
        ...
        if self.writer is not None and self.logger_type in ["wandb", "neptune"]:
            ...
            # Upload params/env.yaml and agent.yaml. wandb.config stores dicts via
            # JSON which loses key order; the on-disk yaml is dumped with
            # sort_keys=False and preserves obs term insertion order. Deploy reads
            # these files to get the canonical obs concatenation order. Use
            # base_path=log_dir so the file appears as "params/env.yaml" in wandb,
            # not just "env.yaml" — downstream consumers depend on the prefix.
            from pathlib import Path
            if self.logger_type == "wandb":
                import wandb
                for name in ("env.yaml", "agent.yaml"):
                    p = Path(self.log_dir) / "params" / name
                    if p.exists():
                        wandb.save(str(p), base_path=str(self.log_dir), policy="now")
